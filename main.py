...
class PomodoroTimer:
    def __init__(self):
            ...
            self.root.tk.call('wm', 'iconphoto', self.root._w, PhotoImage(file="images.png"))

            self.s = ttk.Style()
            self.s.configure("TNotebook" , font=("ubuntu",16))
            ...
            self.tabs.add(self.tab1, text="Pomodoro Timer")
    ...
    def start_timer_thread(self):
        if not self.running:
            self.running = True
            t = threading.Thread(target=self.stop_timer_thread)
            t.start()
    ...
    def reset_clock(self):
        ...
        self.short_break_label.config(text="5:00")
